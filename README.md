...
Next install and activate Node (`npm` is included): 
```
...
```
Once npm is available, install Codex CLI with 
...
Once you have codex installed, it will probably prompt you to log in using your ChatGPT account. We'll all have licensed versions of that soon! In the meantime I'm not certain whether a free account will work or not.

If codex is working, then add this to  ~/.codex/config.toml file: 
```
[mcp_servers.palmetto-bio]
command = "/project/cehrett/bio_llms/BioCodex/.venv/bin/python"
args = ["-m", "palmetto_mcp.bio_server"]
env = {PYTHONPATH = "/project/cehrett/bio_llms/BioCodex:/project/cehrett/bio_llms/BioCodex/src", MCP_STDOUT_GUARD_MODE = "detect", PALMETTO_TOOL_PYTHON = "/project/cehrett/bio_llms/Biomni/.venv/bin/python"}

[mcp_servers.palmetto-slurm]
command = "/project/cehrett/bio_llms/BioCodex/.venv/bin/python"
args = ["-m", "palmetto_mcp.slurm_server"]
env = {PYTHONPATH = "/project/cehrett/bio_llms/BioCodex:/project/cehrett/bio_llms/BioCodex/src", MCP_STDOUT_GUARD_MODE = "detect", PALMETTO_TOOL_PYTHON = "/project/cehrett/bio_llms/Biomni/.venv/bin/python"}

[mcp_servers.palmetto]
command = "/project/cehrett/bio_llms/BioCodex/.venv/bin/python"
args = ["-m", "palmetto_mcp.server"]
env = {PYTHONPATH = "/project/cehrett/bio_llms/BioCodex:/project/cehrett/bio_llms/BioCodex/src", MCP_STDOUT_GUARD_MODE = "detect", PALMETTO_TOOL_PYTHON = "/project/cehrett/bio_llms/Biomni/.venv/bin/python"}

[model_providers.palmetto-llm]
name = "Palmetto LLM Gateway"
base_url = "https://llm.rcd.clemson.edu/v1"
env_key = "PALMETTO_LLM_API_KEY"
wire_api = "chat"

[profiles.palmetto-oss-20b]
model = "gptoss-20b"
model_provider = "palmetto-llm"

[profiles.palmetto-oss-120b]
model = "gptoss-120b"
model_provider = "palmetto-llm"

[profiles.palmetto-lm-4.5-air-fp8]
model = "glm-4.5-air-fp8"
model_provider = "palmetto-llm"

[profiles.palmetto-ruct-fp8]
model = "qwen3-30b-a3b-instruct-fp8"
model_provider = "palmetto-llm"
```

You'll also need to add an API key from Doug's proxy server (https://llm.rcd.clemson.edu/ui/) and expose it as `PALMETTO_LLM_API_KEY`, e.g. by adding `export PALMETTO_LLM_API_KEY=$(cat [a text file containing your API key])` to your .bashrc. add an API key from Doug's proxy server and expose it as `PALMETTO_LLM_API_KEY`, e.g. by adding this to your .bashrc:
